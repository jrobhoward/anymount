...
//! cargo feature. Because cargo cannot express per-OS defaults, all three
//! features are on by default and simply compile to nothing off-platform; the
//! dependencies themselves live under `[target.'cfg(...)'.dependencies]`, so a
//! Linux build never fetches the `windows` crate and vice versa.
...
pub(crate) fn mount<F: ReadOnlyFs>(builder: MountBuilder, fs: F) -> Result<Mount> {
    ...
    let inner = match requested {
        ...
        #[cfg(all(windows, feature = "projfs"))]
        Backend::ProjFs => MountHandle::ProjFs(projfs::mount(builder, fs)?),
...
///
/// Windows prefers cfapi when available: it needs no one-time admin feature
/// enable, can stream without persisting data to disk, and dehydrates
/// automatically. ProjFS is the fallback. Phase 0 of the plan determines
/// whether cfapi can register a sync root from an unpackaged binary; until that
/// is settled, ProjFS is tried first.
...
fn auto_mount<F: ReadOnlyFs>(builder: MountBuilder, fs: F) -> Result<MountHandle> {
    ...
    #[cfg(all(windows, feature = "projfs"))]
    {
        return Ok(MountHandle::ProjFs(projfs::mount(builder, fs)?));
    }

    #[cfg(all(windows, feature = "cfapi", not(feature = "projfs")))]
    {
    ...
    Err(FsError::Unsupported(
        "no mount backend compiled in for this platform; \
         enable the `fuse` feature on Linux/macOS or `projfs`/`cfapi` on Windows",
    ))
}
...
fn unavailable(backend: Backend) -> FsError {
    match backend {
        ...
        Backend::ProjFs => {
            FsError::Unsupported("the `projfs` backend requires Windows and the `projfs` feature")
        }
        Backend::CfApi => {
